...
from ..models import Alert, Chore, Event, User
...
class FamilyAIAgent:
    ...
    def _call_llm(self, prompt: str):
        try:
            response = requests.post(
                self.ollama_url, json={"model": settings.OLLAMA_MODEL, "prompt": prompt, "stream": False}, timeout=30
            )
    ...
    def analyze_user_schedule(self, user_id: int):
        ...
        events = (
            self.db.query(Event)
            .filter(
                Event.user_id == user_id, Event.start_time >= now.isoformat(), Event.start_time <= today_end.isoformat()
            )
            .all()
        )

        chores = self.db.query(Chore).filter(Chore.assignee_id == user_id, Chore.is_completed == False).all()
        ...
        if count > user.threshold_preference:
            ...
            existing = (
                self.db.query(Alert)
                .filter(Alert.user_id == user_id, Alert.created_at >= now.replace(hour=0, minute=0, second=0))
                .first()
            )
